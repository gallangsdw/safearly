package com.asyikwae.safearly.auth
        ...
        if (requestCode == RC_SIGN_IN) {
            ...
            if (task.isSuccessful) {
                try {
                    ...
                    Log.d("auth activity", "firebaseAuthWithGoogle:" + account.id)
                    firebaseAuthWithGoogle(account.idToken!!)
                } catch (e: ApiException) {
                    // Google Sign In failed, update UI appropriately
                    Log.w("auth activity", "Google sign in failed", e)
                }
            } else {
                Log.w("auth activity", exception.toString())
            }
        }
    }
